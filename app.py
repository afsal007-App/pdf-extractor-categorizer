...
with tabs[0]:
    ...
    if uploaded_pdfs:
        ...
        if all_transactions:
            ...
            # Store converted file in session state
            ...
            output.seek(0)
            st.download_button("📥 Download Excel", data=output, file_name="converted_transactions.xlsx")
        else:
...
with tabs[1]:
    ...
    if master_df.empty:
        ...
        # Include converted file from previous section if available
        ...
        if files_to_categorize:
            ...
            for file in files_to_categorize:
                ...
                if desc_col:
                    ...
                    st.error(f"⚠️ No description column found in {file if isinstance(file, pd.DataFrame) else file.name}.")
